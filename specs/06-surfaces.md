...
**Core is type; chrome is term.** The panel must always exist with its core functionality intact (approvals, registry/audit, intake, parameters — and above all the what-will-execute commit paths). On top of that it is **embeddable and customizable**: its views mount inside custom dashboards, and the term may re-skin, rearrange, extend with its own pages/tiles, or wrap it entirely (the FPD-style dashboard with the panel inside). Customization lives in term surfaces — no plugin framework in the type; the commit paths and server-rendered approval views are never overridable, so however deeply it's customized, what-you-approve remains what-executes.
...
Dashboards are **term components** (P11) and may wear two hats: a surface (data out) and, via a second registration, a window (data in — e.g. an FPD-style dashboard whose daily-log entry box writes `capture()`). Each hat gets its own narrow grant; the handshake law covers both. Panel views mount inside them, customized per §Panel — the term decides the chrome, the type keeps the commit paths.
