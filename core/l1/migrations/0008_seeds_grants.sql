...
insert into l1.kinds (domain, key, description) values
  ...
  ('notable_reason','milestone','A goal achieved or marker passed. e.g. first paper submission accepted.'),
  ('notable_reason','first_contact','A significant new person or opportunity entered the life. e.g. the Thiel Fellowship reach-out.'),
  ('notable_reason','purpose_advance','Directly advances the purpose contract; an advances link should exist or be created. e.g. a breakthrough on the agents research goal.'),
  ('notable_reason','rare_event','First or rare against the longitudinal record — the frequency claim must survive a COUNT over atoms, never model recall. e.g. first call with Mom in 3 months.'),
  ('notable_reason','relationship_beat','A major moment in a close relationship. e.g. anniversary dinner; the MRI-results conversation.'),
  ('notable_reason','decision','A consequential decision was made. e.g. chose the lab over the internship.'),
  ('notable_reason','emotional_peak','Joy, grief, or spiritual experience the biography should keep. e.g. temple session that resolved a question.'),
  ('notable_reason','user_asserted','The user said so. Always wins; needs no other justification.'),
...
insert into l1.components (id, kind, circle, status, definition_path, trigger, config, reliability) values
  ...
  -- no run row when idle (run-worker exits before start_run), so they get idle-tolerant 3 days
  ('filer', 'gardener', 'inner', 'disabled', 'core/agents/filer',
   '{"type":"query","interval_min":1,"max_silence_min":4320}', '{"model_tier":"frontier"}', 'critical'),
  ('brief', 'workflow', 'inner', 'disabled', 'core/agents/brief',
   '{"type":"cron","schedule":"0 7 * * *","max_silence_min":1560}', '{"model_tier":"cheap","degraded_mode":"deterministic_skeleton"}', 'critical'),
